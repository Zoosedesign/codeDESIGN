...
//--------------- BASIC SLIDER ---------------
//variabili globali
const sliderBasic = document.getElementById('basic-slider');
let vh = 0;

sliders.forEach(slide => {
    sliderBasic.innerHTML += `<img src="${slide.src}" alt="${slide.alt}" class="w-100 vh-100 object-fit-cover" style="object-position: 65%">`
});

function pre() {
    vh == 0 ? vh = -((sliders.length - 1) * 100) : vh = vh + 100;

    sliderBasic.style.transform = `translateY(${vh}vh)`;
}

function post() {
    vh == -((sliders.length - 1) * 100) ? vh = 0 : vh = vh - 100;
...
//----------------- LOOP SLIDER ------------------
//variabili globali
const sliderLoop = document.getElementById('loop-slider');
let timing = 500; //tempo durata effetti di transizione

//popolo lo slider
sliders.forEach((slide, index) => {
    sliderLoop.innerHTML += `<img src="${slide.src}" alt="${slide.alt}" class="w-100 vh-100 position-absolute top-0 object-fit-cover" style="object-position: 65%; z-index: ${sliders.length - index}">`
});

function previous() {
    const slides = sliderLoop.querySelectorAll('img');
    ...
    sliderLoop.insertBefore(lastSlide, sliderLoop.firstChild);
...
function next() {
    ...
    //riordino le slide e modifico gli z-index
    firstSlide.style.zIndex = slides.length - 2;
    firstSlide.parentNode.removeChild(firstSlide);

    sliderLoop.appendChild(firstSlide);
